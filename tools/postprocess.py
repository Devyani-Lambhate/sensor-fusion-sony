...
# ====================== NEW: 3D → 2D Corner Conversion ======================
def boxes3d_to_corners_2d(boxes_3d):
    """
    Convert BEVFusion 3D boxes [x, y, z, w, l, h, sin, cos, ...] 
    to 2D corner format [N, 4, 2] used by the original postprocess code.
    """
    if isinstance(boxes_3d, torch.Tensor):
        boxes_3d = boxes_3d.cpu().numpy()
    
    x, y = boxes_3d[:, 0], boxes_3d[:, 1]
    w, l = boxes_3d[:, 3], boxes_3d[:, 4]   # width (along x), length (along y)
    yaw = np.arctan2(boxes_3d[:, 6], boxes_3d[:, 7])
    ...
    rot = np.stack([cos, -sin, sin, cos], axis=1).reshape(-1, 2, 2)  # (N, 2, 2)
    
    corners = np.einsum('bij,nj->bni', rot, corners_local)   # (N, 4, 2)
    corners += np.stack([x, y], axis=1)[:, None, :]

    return corners  # (N, 4, 2)

def nms_3d_bev(boxes_3d, scores, iou_threshold=0.5):
    """Convenience wrapper for BEVFusion output"""
    corners = boxes3d_to_corners_2d(boxes_3d)   # (N, 4, 2)
    return non_max_suppression(corners, scores, iou_threshold)
