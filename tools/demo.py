...
class DemoDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        if self.ext == '.bin' or self.ext.endswith('.bin'):
            points = np.fromfile(self.sample_file_list[index], dtype=np.float32).reshape(-1, 5)
            # For .pcd.bin files from NuScenes: x, y, z, intensity, ring
            # The model expects: x, y, z, intensity, timestamp
            # We'll use ring as timestamp placeholder since we don't have actual timestamp info
        elif self.ext == '.npy' or self.ext.endswith('.npy'):
